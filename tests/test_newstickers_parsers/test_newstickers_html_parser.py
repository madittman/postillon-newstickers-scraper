...
def get_mocked_parser(website: str) -> NewstickersHtmlParser:
    ...
    with patch("requests.get") as mock_get:
        ...
        return NewstickersHtmlParser(
            url=website
        )  # The mocked URL is the locally stored website
...
def test_post_init() -> None:
    ...
    for website in WEBSITES:
        with open(website, "rb") as file:
            ...
            expected_soup: BeautifulSoup = BeautifulSoup(
...
def test_extract_newsticker_strings() -> None:
    ...
    for website in WEBSITES:
        ...
        expected_newsticker_strings: list[str] = EXPECTED_NEWSTICKER_STRINGS_BY_WEBSITE[
            Path(website).name
        ]
...
def test_get_next_newsticker() -> None:
    ...
    for website in WEBSITES:
        ...
        for newsticker in mocked_parser.get_next_newsticker():
            expected_newsticker_strings: list[str] = (
                EXPECTED_NEWSTICKER_STRINGS_BY_WEBSITE[Path(website).name]
            )
            ...
            assert (
                newstickers_website.number
                == mocked_parser._get_number_from_title_string(expected_title_string)
            )
